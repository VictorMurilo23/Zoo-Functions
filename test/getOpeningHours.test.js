...
describe('Testes da função getOpeningHours', () => {
  ...
  it('Verifica se getOpeningHours retorna um objeto contendo todos os horários caso não seja passado um parâmetro', () => {
    expect(getOpeningHours()).toMatchObject({ Tuesday: { open: 8, close: 6 }, Wednesday: { open: 8, close: 6 }, Thursday: { open: 10, close: 8 }, Friday: { open: 10, close: 8 }, Saturday: { open: 8, close: 10 }, Sunday: { open: 8, close: 8 }, Monday: { open: 0, close: 0 } });
  });
  ...
  it('Verifica se getOpeningHours só aceita dias e horarios que existem', () => {
    expect(() => { // https://medium.com/@afolabiwaheed/how-to-test-a-function-thats-expected-to-throw-error-in-jest-2419cc7c6462 <-- me ajudou a fazer esse teste.
  ...
  it('Verifica se getOpeningHours retorna `The zoo is closed` caso seja passado um horário ou dia em que o zoológico esteja fechado', () => {
    ...
    expect(getOpeningHours('Thursday', '09:59-AM')).toBe(fechado);
    expect(getOpeningHours('Monday', '04:00-PM')).toBe(fechado);
  });
});
